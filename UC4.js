...
class AddressBook {
    ...
    editContact(firstName, lastName, newDetails) {
        ...
        try {
            if (newDetails.firstName) contact.firstName = contact.validateName(newDetails.firstName, "First Name");
            if (newDetails.lastName) contact.lastName = contact.validateName(newDetails.lastName, "Last Name");
            if (newDetails.address) contact.address = contact.validateMinLength(newDetails.address, "Address", 4);
            if (newDetails.city) contact.city = contact.validateMinLength(newDetails.city, "City", 4);
            if (newDetails.state) contact.state = contact.validateMinLength(newDetails.state, "State", 4);
...
try {
    ...
    myAddressBook.editContact("John", "Doe", { phone: "9123456789", email: "john.new@example.com", city: "Houston" });
